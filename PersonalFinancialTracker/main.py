...
class CSV:
    ...
    def get_transactions(cls,start_date,end_date):
        ...
        print("\nSummary: ")
        print(f"The total Income: ${total_income:.2f}")
        print(f"The total Expense: ${total_expense:.2f}")
...
def add():
    CSV.initialize_csv()
    date = get_date(
        "Enter date of the transaction(dd-mm-yyyy) or press ENTER for today's date:",
        allow_default=True
    )
    amount = get_amount()
...
def plot_transaction(df):
    ...
    plt.xlabel("Date")
    plt.ylabel("Amount")
    plt.title("Income and Expense over a time")
    plt.legend()
    plt.grid(True)
    plt.show()
